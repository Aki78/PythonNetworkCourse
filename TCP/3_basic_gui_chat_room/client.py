...
def connect():
    ...
    #Connect only if these conditions are met
    if name and ip and port:
        #try for connection now
...
def verify_connection(name):
    ...
    if flag == 'NAME':
        ...
        if message:
            ...
            #Create a thread to continuously recieve messages from the server
...
disconnect_button = tkinter.Button(info_frame, text="Disconnect", font=my_font, bg=light_green, borderwidth=5, width=10, state=tkinter.DISABLED, command=disconnect)
...
#Output Frame Layout
my_scrollbar = tkinter.Scrollbar(output_frame, orient=tkinter.VERTICAL)
...
send_button = tkinter.Button(input_frame, text="send", borderwidth=5, width=10, font=my_font, bg=light_green, state=tkinter.DISABLED, command=send_message)
